...
# Model Configuration
datagroup: events_raw { sql_trigger:  SELECT max(event) FROM `mocha-demo.ga4_sample.v_ga_events` WHERE DATE(event) = CURRENT_DATE  ;; }
...
explore: ga_events {
  ...
  join: session_facts {
    ...
    sql_on: ${ga_events.unique_session_id} = ${session_facts.unique_session_id} ;;
  }

  join: user_facts {
    view_label: "User Lifetime Values"
    relationship: many_to_one
    sql_on: ${ga_events.user_id} = ${user_facts.user_id} ;;
  }

  join: top_countries {
    sql_on: ${ga_events.country} = ${top_countries.country} ;;
    relationship: many_to_one
  }
}
